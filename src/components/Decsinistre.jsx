...
const S = {
  ...
  ynBtn: (state, type) => ({
    ...
    border: state === type ? (type === "oui" ? "0.5px solid #27ae60" : "0.5px solid #c0392b") : "0.5px solid #ccc",
    ...
    <div style={S.checkGrid}>
      {CIRCUMSTANCES.map((c, i) => (
        <div key={i} style={S.checkItem}>
          <input type="checkbox" id={`${prefix}_${i}`} checked={!!checks[i]}
...
function SketchPad() {
  ...
  const onDown = (e) => {
    ...
    const p = getPos(e); startRef.current = p;
    ...
    <div>
      <div style={S.sketchTools}>
        ...
        <label style={{ fontSize:12, color:"#666", display:"flex", alignItems:"center", gap:4 }}>
          Taille
          <input type="range" min={1} max={20} value={size} onChange={e => setSize(Number(e.target.value))} style={{ width:80 }} />
          <span style={{ fontSize:11 }}>{size}px</span>
        </label>
      </div>
      <canvas ref={canvasRef} style={S.canvas} height={340}
...
function PhotosSection({ photos, setPhotos }) {
  const handleFiles = (files) => {
    Array.from(files).forEach(f => {
      ...
      reader.onload = ev => setPhotos(p => [...p, { url: ev.target.result, name: f.name, size: f.size }]);
  ...
  const btnStyle = {
    ...
    cursor: "pointer", background: "#fff0f0", color: "#c0392b",
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: "#888", marginBottom: 14 }}>
        Prenez des photos des dégâts sur place ou importez depuis votre galerie. Plusieurs photos possibles.
      </p>

      {/* Boutons */}
      <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
        {/* Appareil photo direct (mobile) */}
      ...
      {photos.length > 0 ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 10 }}>
          {photos.map((p, i) => (
            <div key={i} style={{ position: "relative", borderRadius: 10, overflow: "hidden", border: "0.5px solid #ddd", background: "#f9f9f9" }}>
              <img src={p.url} alt={p.name}
                style={{ width: "100%", height: 130, objectFit: "cover", display: "block" }} />
              {/* Bouton supprimer */}
              <button onClick={() => setPhotos(prev => prev.filter((_,idx) => idx !== i))}
                style={{ position:"absolute", top:5, right:5, background:"#c0392b", color:"#fff", border:"none", borderRadius:"50%", width:22, height:22, cursor:"pointer", fontSize:13, lineHeight:1, display:"flex", alignItems:"center", justifyContent:"center" }}>
                ✕
              </button>
              <p style={{ fontSize:10, color:"#888", padding:"4px 6px", margin:0, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>
                {p.name}
              </p>
            </div>
          ))}
        ...
        <div style={{ border:"1.5px dashed #ddd", borderRadius:10, padding:"2rem", textAlign:"center", color:"#bbb", fontSize:13 }}>
          📷 Aucune photo ajoutée
        </div>
      )}

      {photos.length > 0 && (
        <p style={{ fontSize:11, color:"#27ae60", marginTop:10, fontWeight:500 }}>
...
export default function DecSinistre({ onSave }) {
  ...
  const [photos,       setPhotos]       = useState([]);  // ← NOUVEAU
  ...
  const handleSubmit = () => {
    const data = {
      date, heure, lieu, degTiers, temoin,
      nomAssure, profession, tel, adresseAssure,
      circDesc, pvGend, pvPolice,
      conducteur: {
        habituel: condHabituel, reside: condReside,
        dob: condDob, permis: condPermis,
        categorie: condCat, delivreLe: condDelivre, wilaya: condWilaya,
      },
      vehiculeA: vehi_a,
      vehiculeB: vehi_b,
      ...
      faitA, faitLe,
      photos: photos.map(p => ({ url: p.url, name: p.name })),  // ← NOUVEAU
    };
    if (onSave) onSave(data);
    else alert("Déclaration prête à être transmise à l'assureur.");
  };
    ...
    <div style={S.wrap}>
      ...
      <div style={S.section}>
        ...
        <Field label="Description">
          <textarea style={S.textarea} value={circDesc} onChange={e=>setCircDesc(e.target.value)} placeholder="Décrire les circonstances..." />
        </Field>
        <div style={{ marginTop:10 }}>
      ...
      <div style={S.section}>
        <div style={S.sectionTitle}>
          <span style={S.num}>7</span> Croquis de l'accident
          <span style={{ fontSize:11, fontWeight:400, color:"#888", marginLeft:6 }}>— Position des véhicules, tracé des voies, direction</span>
        </div>
        <SketchPad />
      </div>

      {/* 7bis – Photos des dégâts ← NOUVEAU */}
      <div style={S.section}>
        <div style={S.sectionTitle}>
          <span style={{ ...S.num, fontSize:14 }}>📷</span> Photos des dégâts
        </div>
      ...
      <div style={S.section}>
        ...
        <div style={S.grid2}>
          <Field label="Nature et importance des dégâts">
            <textarea style={S.textarea} value={degatsNature} onChange={e=>setDegatsNature(e.target.value)} />
          </Field>
          <Field label="Nom et adresse du propriétaire">
            <textarea style={S.textarea} value={degatsProp} onChange={e=>setDegatsProp(e.target.value)} />
          </Field>
        </div>
      </div>
