...
const EmptyState = ({ message = "Aucune donnée", sub = "Les données apparaîtront après les premières souscriptions" }) => (
...
const ModalSinistre = ({ sinistre, onClose, onUpdate }) => {
  ...
  const PIECES_PREDEFINIES = [
    "Constat amiable signé", "Rapport de police / PV Gendarmerie",
    "Carte grise du véhicule", "Permis de conduire",
    "Photos des dommages", "Factures de réparation",
    "Attestation d'assurance en cours", "Certificat médical (si blessé)",
    "Devis de réparation", "Carte d'identité nationale",
  ];
      ...
      <div className="bg-white w-full max-w-5xl rounded-3xl overflow-hidden shadow-2xl flex flex-col" style={{ maxHeight: '92vh' }}>
        <div className="bg-slate-900 px-8 py-5 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            <Badge color={sinistre.statut === 'VALIDÉ' ? 'green' : sinistre.statut === 'REFUSÉ' ? 'red' : sinistre.statut === 'PIÈCES REQUISES' ? 'amber' : 'blue'}>
              {sinistre.statut || 'EN COURS'}
            </Badge>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[['A', d.vehiculeA], ['B', d.vehiculeB]].map(([label, v]) => (
                <div key={label} className={`rounded-2xl border-2 p-6 ${label === 'A' ? 'border-blue-200 bg-blue-50' : 'border-red-200 bg-red-50'}`}>
                  <h3 className={`text-sm font-black uppercase mb-4 ${label === 'A' ? 'text-blue-800' : 'text-red-800'}`}>Véhicule {label}</h3>
                  {v ? (
                  ...
                  ) : <p className="text-slate-400 text-sm">Aucune information</p>}
                </div>
              ))}
            </div>
          )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {[['A', d.circonstancesA, 'bg-blue-50 border-blue-200 text-blue-800'], ['B', d.circonstancesB, 'bg-red-50 border-red-200 text-red-800']].map(([label, circ, cls]) => (
                  <div key={label} className={`rounded-2xl border-2 p-5 ${cls.split(' ').slice(0,2).join(' ')}`}>
                    <h3 className={`text-xs font-black uppercase mb-3 ${cls.split(' ')[2]}`}>Cases Véhicule {label} ({Array.isArray(circ) ? circ.length : 0})</h3>
              ...
              {d.blesse && (d.blesse.nom || d.blesse.blessures) ? (
                <div className="bg-red-50 border-2 border-red-200 rounded-2xl p-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    <div className="col-span-2">
                      <p className="text-[9px] font-black uppercase tracking-widest text-slate-400">Blessures</p>
          ...
          {tab === 'decision' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { key: 'valide', icon: '✅', label: 'Valider le remboursement', color: 'green', desc: 'Le sinistre est reconnu.' },
                  { key: 'refuse', icon: '❌', label: 'Refuser le sinistre',       color: 'red',   desc: 'Le sinistre est rejeté.' },
                  { key: 'pieces', icon: '📎', label: 'Demander des pièces',       color: 'amber', desc: 'Dossier incomplet.' },
                ].map(opt => (
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    {["Sinistre non couvert","Exclusion contractuelle","Délai dépassé","Faute intentionnelle","Conduite sous alcool","Permis invalide"].map(m => (
                      <button key={m} onClick={() => setMotifRefus(m)}
                        className={`text-left px-3 py-2 rounded-xl border text-[10px] font-bold transition-all ${motifRefus === m ? 'bg-red-500 border-red-500 text-white' : 'bg-white border-red-200 text-red-700 hover:border-red-400'}`}>
                        {m}
                      </button>
                    ))}
                  </div>
              ...
              {decision === 'pieces' && (
                <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-6 space-y-4">
                  ...
                  <div className="flex gap-3">
                    <input type="text" value={nouvellePiece} onChange={e => setNouvellePiece(e.target.value)} onKeyDown={e => e.key === 'Enter' && addPiece()}
                    ...
                    <button onClick={addPiece} className="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-black text-xs uppercase rounded-xl transition-all">+ Ajouter</button>
                  </div>
                </div>
              )}
        ...
        <div className="overflow-y-auto flex-1 px-8 py-6">
          ...
          {tab === 'garanties' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {d.garanties && [
                { key:'rc', label:'RC – Responsabilité Civile' }, { key:'dr', label:'DR – Défense et Recours' },
                { key:'bdg', label:'BDG – Bris de Glace' },       { key:'vol', label:'VOL – Vol du Véhicule' },
                { key:'inc', label:'INC – Incendie' },             { key:'dc', label:'DC – Dommages Collision' },
                { key:'pt', label:'PT – Personnes Transportées' }, { key:'ir', label:'IR – Individuelle Conducteur' },
          ...
          {tab === 'quittance' && (
            <div className="bg-slate-900 rounded-2xl p-6 text-white space-y-2">
              ...
              {[
                ['Prime de Base', q.primeBase], ['+ Garanties Facultatives', q.totalOptions],
                ['Prime Nette (HT)', q.primeNette, true], ['Total Taxes', q.totalTaxes, true],
                ['Timbre de Dimension', q.timbreDimension], ['Timbre Gradué', q.timbreGradue],
              ].map(([l, v, a]) => (
  ...
  <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden hover:border-slate-300 transition-all">
    <div className="px-6 py-4 flex flex-wrap items-center gap-4">
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-lg ${
        ...
        s.statut === 'PIÈCES REQUISES' ? 'bg-amber-100 text-amber-700' : 'bg-orange-100 text-orange-700'
      }`}>⚠</div>
      ...
      <Badge color={s.statut === 'VALIDÉ' ? 'green' : s.statut === 'REFUSÉ' ? 'red' : s.statut === 'PIÈCES REQUISES' ? 'amber' : 'blue'}>
        {s.statut || 'EN COURS'}
      </Badge>
              ...
              <>
                ...
                <div className="bg-[#0b1120] rounded-3xl p-8 shadow-xl text-white">
                  ...
                  <div className="space-y-3">
                    ...
                    {stats.parRegion.Sud > stats.total * 0.2 && <AlertCard type="info"   message="Présence Zone Sud significative — coefficient 0.80 appliqué." />}
                    {sinistresStats.cours > 0                && <AlertCard type="danger"  message={`${sinistresStats.cours} sinistre(s) en attente de traitement.`} />}
                    {sinistresStats.totalRembourse > 0       && <AlertCard type="info"   message={`Total remboursé : ${formatNum(sinistresStats.totalRembourse)} DZD`} />}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {['Toutes','Nord','Est','Ouest','Sud'].map(r => (
                <button key={r} onClick={() => setFilterRegion(r)}
                  className={`px-5 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${filterRegion === r ? 'bg-orange-500 text-white shadow-md' : 'bg-white text-slate-500 border-2 border-slate-200 hover:border-orange-300'}`}>{r}</button>
              ))}
            </div>
            {dossiers.length === 0 ? <EmptyState message="Aucun dossier" sub="Les dossiers apparaîtront après les premières souscriptions" /> :
              ...
              {['Tous','EN COURS','VALIDÉ','REFUSÉ','PIÈCES REQUISES'].map(f => (
                <button key={f} onClick={() => setFilterSinistre(f)}
                  className={`px-4 py-3 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${
                    filterSinistre === f
                      ? f === 'VALIDÉ' ? 'bg-green-500 text-white' : f === 'REFUSÉ' ? 'bg-red-500 text-white' : f === 'PIÈCES REQUISES' ? 'bg-amber-500 text-white' : f === 'EN COURS' ? 'bg-blue-500 text-white' : 'bg-slate-900 text-white'
                      : 'bg-white text-slate-500 border-2 border-slate-200 hover:border-slate-400'
                  }`}>{f}</button>
              ))}
            </div>
            {sinistresList.length === 0 ? <EmptyState message="Aucun sinistre" sub="Les sinistres apparaîtront après les premières déclarations" /> :
